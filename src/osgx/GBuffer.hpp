...
// ================================================================================================
// osgx::gbuffer -- generic deferred G-buffer camera setup. Not PBR/glTF-specific: the proven MRT
// shape `examples/pyosg-mrt.py` validated (OpenSceneGraph.py, 2026-07-11 -- one geometry pass
// writing several SIMULTANEOUS color attachments via `layout(location = n) out`, plus a real
// depth attachment), generalized from that example's fixed two-color-buffer toon-shading demo
// into a reusable helper any deferred consumer can call -- osgx::gltf::pbribl's own G-buffer
// split (PBRIBL.hpp's createPBRIBLGeometryPass()) is built on top of this, not a separate
// mechanism, and a non-PBR deferred shader (e.g. a toon pipeline like pyosg-mrt.py's own) can
// use it directly too.
// ================================================================================================
...
// (indexed exactly as passed to createGBufferCamera(), i.e. colorTextures[i] is
...
// caller's own fragment shader, one per format in order) plus a real GL_DEPTH_COMPONENT24 depth
// attachment. `referenceFrame` defaults to RELATIVE_RF (composes with `node`'s own ancestor
// transforms/camera, the normal case for a G-buffer pass parented under the real scene) --
// pass ABSOLUTE_RF explicitly for a camera that should own its own fixed view/projection
// instead (e.g. a shadow-map-style pass; osgx::shadow::createDirectionalShadowMap() does this
