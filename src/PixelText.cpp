...
// pyosg_dice.py's FONT_5X7, extended from digits-only to PixelText::CHARSET. Digit shapes are
// copied verbatim from FONT_5X7 so an eventual dice switch-over renders pixel-identically.
...
const std::unordered_map<char, Glyph>& glyphTable() {
	static const std::unordered_map<char, Glyph> table = {
		...
		// Digit shapes copied verbatim from pyosg_dice.py's FONT_5X7.
		{'1', {"..#..", ".##..", "..#..", "..#..", "..#..", "..#..", ".###."}},
		{'2', {".###.", "#...#", "....#", "...#.", "..#..", ".#...", "#####"}},
		{'3', {"####.", "....#", "...#.", "..##.", "....#", "....#", "####."}},
		{'4', {"...#.", "..##.", ".#.#.", "#..#.", "#####", "...#.", "...#."}},
...
// Uppercase-folds `c`, falling back to space for anything outside PixelText::CHARSET - the
// forgiving policy PixelText::setText() wants. createAtlas() validates (and throws) separately,
// before any character reaches this.
char normalizeChar(char c) {
	char upper = static_cast<char>(std::toupper(static_cast<unsigned char>(c)));

	return PixelText::CHARSET.find(upper) != std::string_view::npos ? upper : ' ';
}
...
osg::ref_ptr<osg::Image> createCharsetAtlas(int pixelScale=7) {
	...
	// height (square cells), so it must be sized off GLYPH_ROWS (7), not GLYPH_COLS (5) - a
...
osg::ref_ptr<osg::Image> PixelText::createAtlas(
	...
	for(const auto& cell : cells) {
		for(char c : cell) {
			...
			if(CHARSET.find(upper) == std::string_view::npos) {
